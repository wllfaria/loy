...
static TypedLetBinding* typer_typecheck_let_binding(
    ...
    if(!typer_type_equals(expected_type, actual_type)) {
        fprintf(
            stderr,
            "Type error: expected `%s` but got different type in let binding\n",
            type_name
        );
        exit(EXIT_FAILURE);
    }
...
static TypedInt* typer_typecheck_int(
    ...
    if(!expected->is_signed) {
        fprintf(
            stderr,
            "Error: signed literal cannot be assigned to u%u\n",
            expected->bit_width
        );
        exit(EXIT_FAILURE);
    }
    ...
    if(value > (i64)max) {
        fprintf(
            stderr,
            "Error: int literal %ld too large for i%u.\n",
            value,
            expected->bit_width
        );
        exit(EXIT_FAILURE);
    ...
    if(value < min) {
        fprintf(
            stderr,
            "Error: int literal %ld too small for i%u.\n",
            value,
            expected->bit_width
        );
        exit(EXIT_FAILURE);
...
static TypedNode* typer_typecheck_uint(
    ...
    if(expected->is_signed) {
        if(value > (u64)max_int_value(expected->bit_width)) {
            fprintf(
                stderr,
                "Error: unsigned literal %lu too large for i%u.\n",
                uint_node->value,
                expected->bit_width
            );
            exit(EXIT_FAILURE);
    ...
    if(value > max_uint_value(expected->bit_width)) {
        fprintf(
            stderr,
            "Error: unsigned literal %lu too large for u%u.\n",
            uint_node->value,
            expected->bit_width
        );
        exit(EXIT_FAILURE);
...
static TypedFunCall* typer_typecheck_fun_call(
    ...
    FunctionDecl* signature = hash_map_get(
        &ctx->functions,
        fun_name,
        strlen(fun_name)
    );
    ...
    if(signature == NULL) {
        fprintf(
            stderr,
            "Error: calling undeclared function %s\n",
            fun_name
        );
        exit(EXIT_FAILURE);
    }
...
static TypedIdent* typer_typecheck_ident(
    ...
    if(type == NULL) {
        type = hash_map_get(&ctx->type_decl, type_name, strlen(type_name));
    }
...
static TypedFunction* typer_typecheck_function(
    ...
    while(vector_iter_peek(&body_iter) != NULL) {
        ...
        case AST_NODE_LET_BINDING: {
            ...
            vector_push_ptr(&typed_body, binding);
            break;
        }
        case AST_NODE_FUN_CALL:
        case AST_NODE_IDENTIFIER:
            UNREACHABLE("identifiers are not allowed here");
        case AST_NODE_FUN:
            UNREACHABLE("function declarations are not allowed here");
        case AST_NODE_FUN_ARG:
            UNREACHABLE("function args are not allowed here");
        case AST_NODE_BINARY_OP:
            UNREACHABLE("binary ops are not allowed here");
        case AST_NODE_TYPE_ANNOTATION:
            UNREACHABLE("type annotations are not allowed here");
        case AST_NODE_INT_LITERAL:
            UNREACHABLE("int literal is not allowed here");
        case AST_NODE_UINT_LITERAL:
            UNREACHABLE("uint literal is not allowed here");
        case AST_NODE_FLOAT_LITERAL:
            UNREACHABLE("float literal is not allowed here");
            break;
        }
    }
...
char* typer_fmt_node(Allocator* allocator, void* item, u64 indentation) {
    ...
    case TYPED_NODE_FUN: {
        ...
        if(fun->body.len > 0) string_builder_write_string(&builder, "\n");
        ...
        indentation--;
        if(fun->body.len > 0) string_builder_indent(&builder, indentation);
        string_builder_write_string(&builder, "],\n");
